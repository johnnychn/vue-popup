    ...
    <div class="vue-popup" v-show="pop" :style="{'z-index': zIndex}">
        ...
        <div class="vue-popup-scroll" @click="maskClick()" v-show="show" :transition="panel_transition">
            <div class="vue-popup-panel" :class="{'vue-popup-shake-animation':shake==true}"
                ...
                <div class="content" :style="{maxHeight:maxHeight,overflow:'auto',height:size.height+'px',width:size.width+'px'}">
                    <slot>{{{content}}}</slot>
                </div>
    ...
    export default {
        ...
        watch: {
            ...
            'size.width':function (val,oldVal) {
                this.popupPanelStyle.width=parseInt(val)+'px';
                console.log( this.popupPanelStyle)
                this.updateSize();
            },
