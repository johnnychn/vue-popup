...
<template>
    <div class="vue-popup" v-show="pop" :style="{'z-index': zIndex}">
        ...
        <div class="vue-popup-scroll" @click="maskClick()" v-show="show" :transition="panel_transition">
            <div class="vue-popup-panel t300" :class="{'vue-popup-shake-animation':shake==true}"
                ...
                <div class="content t300"
                     :style="{maxHeight:(maxHeight>0?(maxHeight+'px'):''),overflow:'auto',height:size.height+'px',width:size.width+'px'}">
                    <slot>{{{content}}}</slot>
                </div>
...
/*!
* vue-popup v0.0.2(https://github.com/johnnyGoo/vue-popup)
* Author: Johnny chen
*
...
<script>
    ...
    export default {
        ...
        props: {
            ...
            popupPanelStyle: {
                type: Object, default: function () {
                    return {background: '#fff', boxShadow: '0 2px 3px #666', width: '400px'}
                }
            },
            cancelStyle: {
                type: Object, default: function () {
        ...
        methods: {
            ...
            updateSize: function () {
                if (this.show == true) {
                    ...
                    var css = {'margin-top': 0, 'overflow': 'hidden'};
                    //css['width'] = popSize.width;

                    var ph = popSize.height
                    if (this.maxHeight && this.maxHeight > 0) {
                        ph = Math.min(ph, this.maxHeight);
                    }


                    if (ph < windowSize.height - 60) {


                        css['margin-top'] = (windowSize.height - ph) / 2;
        ...
        created: function () {
            ...
            this.popupPanelStyle.width = this.size.width + 'px';
            if (this.show) {
                this.show = false;
                var me = this;
        ...
        watch: {
            ...
            'size.width': function (val, oldVal) {
                this.popupPanelStyle.width = Math.ceil(val) + 'px';
                this.updateSize();
            },
            'size.height': function (val, oldVal) {
                this.updateSize();
            }
        }
